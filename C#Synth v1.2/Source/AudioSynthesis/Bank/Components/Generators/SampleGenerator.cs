        ...
        private void Interpolate(GeneratorParameters generatorParams,float[] blockBuffer,double increment,int start,int end)
        {
            switch(Synthesizer.InterpolationMode)
            {
                case InterpolationEnum.Linear:
                    #region Linear
                    {
                        double _end = generatorParams.currentState == GeneratorStateEnum.Loop ? loopEnd - 1 : this.end - 1;
                        int index;
                        float s1, s2, mu;
                ...
                case InterpolationEnum.CubicSpline:
                    #region CubicSpline
                    {
                        double _end = generatorParams.currentState == GeneratorStateEnum.Loop ? loopStart + 1 : this.start + 1;
                        int index;
                        ...
                        _end = generatorParams.currentState == GeneratorStateEnum.Loop ? loopEnd - 2 : this.end - 2;
