...
namespace DirectSoundDemo
{
    ...
    {
        private delegate void timedelegate(int current, int max, int voices);
        ...
        public void updateTime(int current, int max, int voices)
        {
            if (trackBar1.InvokeRequired || label1.InvokeRequired)
                this.BeginInvoke(tfunc, new object[] {current, max, voices});
            else
                updateTimeControls(current, max, voices);
        }
        private void updateTimeControls(int current, int max, int voices)
        {
            ...
            this.Text = "Play Controls VoiceCount: " + voices; 
        }
        public int getTime()
        {
